...
export async function getPresupuestoById(id) {
  ...
  const presupuestoCheck = await query(`SELECT id FROM presupuestos WHERE id = ?`, [id])
  ...
  console.log("getPresupuestoById - Resultados encontrados:", presupuestos.length)
  ...
  console.log("getPresupuestoById - ID del presupuesto encontrado:", presupuesto.id)
...
// Actualizar la función createPresupuesto para manejar el IVA
export async function createPresupuesto(data) {
  const { cliente, productos, subtotal, iva_monto, total, iva_porcentaje } = data
  ...
  try {
    ...
    let clienteId
    const clientesExistentes = await connection.execute("SELECT id FROM clientes WHERE nombre = ?", [cliente.nombre])

    if (clientesExistentes[0].length > 0) {
      clienteId = clientesExistentes[0][0].id

      // Actualizar datos del cliente
      await connection.execute("UPDATE clientes SET domicilio = ?, entre_calles = ?, telefono = ? WHERE id = ?", [
        cliente.domicilio,
        cliente.entreCalles,
        cliente.telefono,
        clienteId,
      ...
      clienteId = nuevoCliente[0].insertId
    }
    ...
    const presupuestoResult = await connection.execute(
      ...
      [numero, clienteId, subtotal, iva_monto, total, iva_porcentaje, "presupuesto", fechaCreacion],
    )
    ...
    for (const producto of productos) {
      await connection.execute(
        ...
        [presupuestoId, producto.nombre, producto.cantidad, producto.precioUnitario, producto.subtotal],
      )
    }
...
export async function updatePresupuesto(id, data) {
  ...
  try {
    ...
    const presupuestoExistente = await connection.execute("SELECT id, cliente_id FROM presupuestos WHERE id = ?", [id])
    ...
    // Actualizar datos del cliente
    await connection.execute(
      "UPDATE clientes SET nombre = ?, domicilio = ?, entre_calles = ?, telefono = ? WHERE id = ?",
      [cliente.nombre, cliente.domicilio, cliente.entreCalles, cliente.telefono, clienteId],
    )

    // Actualizar presupuesto
    await connection.execute(
      "UPDATE presupuestos SET subtotal = ?, iva_monto = ?, total = ?, iva_porcentaje = ? WHERE id = ?",
      [subtotal, iva_monto, total, iva_porcentaje, id],
    )

    // Eliminar productos existentes
    await connection.execute("DELETE FROM presupuesto_productos WHERE presupuesto_id = ?", [id])

    // Insertar nuevos productos
    for (const producto of productos) {
      await connection.execute(
        "INSERT INTO presupuesto_productos (presupuesto_id, nombre, cantidad, precio_unitario, subtotal) VALUES (?, ?, ?, ?, ?)",
        [id, producto.nombre, producto.cantidad, producto.precioUnitario, producto.subtotal],
      )
    }
...
export async function deletePresupuesto(id) {
  ...
  try {
    // Verificar si el presupuesto existe
    const presupuestoExistente = await connection.execute("SELECT id FROM presupuestos WHERE id = ?", [id])
...
export async function convertirPresupuestoAPedido(id, estadoPago = "a_pagar", montoRestante = 0, fechaEntrega = null) {
  // Iniciar transacción
  const connection = await pool.getConnection()
  await connection.beginTransaction()
...
export async function getPedidoById(id) {
  ...
  const pedidoCheck = await query(`SELECT id FROM presupuestos WHERE id = ? AND estado = 'pedido'`, [id])
...
export async function actualizarEstadoPedido(id, data) {
  const { estado_entrega, chofer, estado_pago, monto_restante, fecha_entrega } = data
  ...
  try {
    // Verificar si el pedido existe
    const pedidoExistente = await connection.execute('SELECT id FROM presupuestos WHERE id = ? AND estado = "pedido"', [
      id,
    ])
    ...
    // Imprimir la fecha de entrega para depuración
    console.log("Fecha de entrega recibida en actualizarEstadoPedido:", fecha_entrega)
    ...
    if (fecha_entrega) {
      ...
      if (fecha.getFullYear() !== añoActual) {
        ...
        // Formatear la fecha como YYYY-MM-DD
        fechaEntregaCorregida = fecha.toISOString().split("T")[0]
        console.log("Fecha de entrega corregida en actualizarEstadoPedido:", fechaEntregaCorregida)
      }
    }
    ...
    await connection.execute(
      ...
      [estado_entrega, chofer, estado_pago, monto_restante, fechaEntregaCorregida, id],
    )
...
export async function getPedidosByFecha(fecha) {
  ...
  try {
    ...
    // Consulta SQL que compara solo la parte de fecha (sin hora)
...
// Modificar la función getPedidosProgramados para corregir el manejo de fechas
export async function getPedidosProgramados(fechaDesde = null) {
  console.log("Ejecutando búsqueda de pedidos programados desde fecha:", fechaDesde)
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      console.log("Último resultado fecha_entrega:", results[results.length - 1].fecha_entrega)
    }
