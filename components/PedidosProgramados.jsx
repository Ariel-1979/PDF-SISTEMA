...
import { Calendar, FileText, ChevronDown, ChevronUp, Eye, Edit } from "lucide-react"
...
const PedidosProgramados = () => {
  ...
  const fetchPedidosProgramados = async () => {
    try {
      ...
      const res = await fetch(`/api/pedidos/programados?fechaDesde=${formattedToday}`)

      if (!res.ok) {
        const errorData = await res.json()
        throw new Error(errorData.error || "Error al cargar pedidos programados")
      }
      ...
      showToast("Error al cargar pedidos programados: " + error.message, "error")
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="pedidos-programados">
          {fechasOrdenadas.map((fecha) => (
            <div key={fecha} className="fecha-grupo">
              <div className="fecha-header" onClick={() => toggleDateExpand(fecha)}>
                <div className="fecha-info">
                  ...
                  <span className="fecha-count">{pedidosPorFecha[fecha].length} pedido(s)</span>
                </div>
                {expandedDates[fecha] ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
              </div>
                ...
                <div className="pedidos-list">
                  ...
                  {pedidosPorFecha[fecha].map((pedido) => (
                    <div key={pedido.id} className="pedido-item">
                      ...
                      <div className="pedido-column" data-label="Estado Entrega">
                      ...
                      <div className="pedido-column pedido-actions" data-label="Acciones">
                        <Link
