...
const PedidoDetail = ({ pedido }) => {
  ...
  const [estadoEntrega, setEstadoEntrega] = useState(pedido?.estado_entrega || "pendiente")
  const [estadoPago, setEstadoPago] = useState(pedido?.estado_pago || "a_pagar")
  const [montoRestante, setMontoRestante] = useState(pedido?.monto_restante || 0)
  ...
  const generarPDF = async () => {
    ...
    // Fecha
    const fecha = new Date(pedido.fecha_conversion || pedido.fecha_creacion).toLocaleDateString()
    doc.text(`Fecha: ${fecha}`, 150, 70)

    // Fecha de entrega
    if (pedido.fecha_entrega) {
      doc.text(`Fecha de entrega: ${new Date(pedido.fecha_entrega).toLocaleDateString()}`, 150, 78)
    } else {
      doc.text(`Fecha de entrega: Pendiente`, 150, 78)
    }

    // Estado de pago
    doc.text(`Estado de pago: ${getEstadoPagoText(pedido.estado_pago)}`, 150, 86)

    if (pedido.estado_pago === "resta_abonar") {
      doc.text(`Monto restante: $${Number(pedido.monto_restante).toLocaleString()}`, 150, 94)
    }
    ...
    doc.text(`Subtotal: $${Number(subtotal).toLocaleString()}`, 150, finalY + 10)

    if (Number.parseFloat(ivaPorcentaje) > 0) {
      doc.text(`IVA (${ivaPorcentaje}): $${Number(ivaMonto).toLocaleString()}`, 150, finalY + 18)
    ...
    doc.text("Los materiales o productos de segunda selección, no tienen cambio, ni devolución.", 20, finalY + 45)
    ...
    try {
      ...
      if (fechaEntrega) {
        ...
        if (fecha.getFullYear() !== añoActual) {
          ...
          console.log("Fecha de entrega corregida en handleSaveChanges:", fechaEntregaCorregida)
        }
      }
      ...
      console.log("Enviando datos para actualizar estado:", JSON.stringify(data))
      ...
      <div className="pedido-detail-header">
        ...
        <div className="pedido-detail-actions">
          ...
          <Link
            ...
            onClick={() => console.log("Navegando a editar pedido con ID:", pedidoId)}
          >
            <Edit size={20} />
            <span>Editar Pedido</span>
          </Link>
          {!editing ? (
            <button className="btn btn-primary btn-icon" onClick={() => setEditing(true)}>
              <Edit size={20} />
              <span>Editar Estado</span>
            </button>
          ) : (
            <button className="btn btn-primary btn-icon" onClick={handleSaveChanges} disabled={saving}>
              <Save size={20} />
      ...
      <div className="pedido-detail-info">
        ...
        <div className="pedido-detail-section">
          ...
          <div className="pedido-detail-meta">
            <p>
              <strong>Fecha de creación:</strong> {new Date(pedido.fecha_creacion).toLocaleDateString()}
            </p>
            <p>
              <strong>Fecha del pedido:</strong>{" "}
              {pedido.fecha_conversion ? new Date(pedido.fecha_conversion).toLocaleDateString() : "N/A"}
            </p>
            <p>
            ...
            {!editing ? (
              <>
                <p>
                  <strong>Estado de entrega:</strong> {getEstadoEntregaText(pedido.estado_entrega)}
                </p>
                ...
                <p>
                  <strong>Estado de pago:</strong> {getEstadoPagoText(pedido.estado_pago)}
                </p>
                {pedido.estado_pago === "resta_abonar" && (
                  <p>
                    <strong>Monto restante:</strong> ${Number(pedido.monto_restante).toLocaleString()}
                  </p>
                )}
              </>
            ) : (
        ...
        <div className="pedido-detail-table">
          ...
          {pedido.productos.map((producto) => (
            <div key={producto.id} className="pedido-detail-table-row">
              ...
              <div className="pedido-detail-column product-name" data-label="Producto">
                {producto.nombre}
              </div>
              <div className="pedido-detail-column" data-label="Precio Unitario">
                ${Number(producto.precio_unitario).toLocaleString()}
              </div>
          ...
          <div className="pedido-detail-table-footer">
            <div className="pedido-detail-total">
              ...
              {Number.parseFloat(ivaPorcentaje) > 0 && (
                <p>
                  <strong>IVA ({ivaPorcentaje}):</strong> ${Number(ivaMonto).toLocaleString()}
                </p>
              )}
