...
const PresupuestoForm = () => {
  ...
  const generarPDF = () => {
    ...
    doc.autoTable({
      ...
      didDrawPage: (data) => {
        // Solo en la última página agregamos total y términos
        if (doc.internal.getNumberOfPages() === data.pageNumber) {
          const finalY = data.cursor.y + 10;
          doc.setFontSize(12);
          doc.text(`Total: $${calcularTotal().toLocaleString()}`, 150, finalY);

          doc.setFontSize(8);
          let y = finalY + 20;
          doc.text("Este presupuesto tiene una validez de 24 hs.", 20, y);
          doc.text(
