...
import { createPresupuesto, getAllPresupuestos, searchPresupuestos } from "@/lib/db"

// Modificar la función GET para agregar el parámetro limit
...
export async function POST(request) {
  try {
    ...
    if (!data.cliente) {
      return NextResponse.json({ error: "Datos de cliente no proporcionados" }, { status: 400 })
    }

    if (!Array.isArray(data.productos) || data.productos.length === 0) {
      return NextResponse.json({ error: "Lista de productos inválida o vacía" }, { status: 400 })
    }
